...
export default {
  ...
  execute: async (message, client, payload) => {
    ...
    const user = (await db.select().from(student).where(eq(student.phone, student_phone)).limit(1))[0];

    let res = `👤 *PROFIL AUTENTIKASI: ${student_phone}*\n`;
    ...
    },
    {
      ...
      execute: async (message, client, payload) => {
        ...
        client.messageClient.sendMessage(remoteJid, {
          text: `✅ Nickname berhasil diperbarui: *${input}*.\n\n${footer}`,
        });
      },
    },
    {
      ...
      execute: async (message, client, payload) => {
        ...
        if (userData.length == 0) {
          return client.messageClient.sendMessage(message.key?.remoteJid!, {
            text: `🚫 Data Anda tidak ditemukan. Silakan daftarkan diri terlebih dahulu.`,
          });
        }
