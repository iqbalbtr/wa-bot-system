  ...
  commands: [
    ...
    },
    {
      ...
      execute: async (msg, client, payload, data) => {
        ...
        const sheetsId = await getSheets(result.category, result.title, result.date || "penilaian", true);
        ...
        const challange_record = await db
          ...
          .where(
            and(
              ...
              eq(chalangeStudent.challange_title, result.title),
            ),
          );
    ...
    },
    {
      ...
      execute: async (msg, client, payload, data) => {
        ...
        const { category, date, title } = result;

        const sheetsId = await getSheets(category, title, date || "penilaian");
        ...
        return client.messageClient.sendMessage(remoteJid, {
          text: `✅ *SUKSES:* Data tantangan untuk ${result.title} berhasil disimpan ke database. ${challenge_record.length !== sheetsTable.values.length - 1 ? "terdapat perbedaan jumlah data. update dengan /edit untuk mendapatkan data terbaru" : ""}`,
        });
      },
    },
    {
